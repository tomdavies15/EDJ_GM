...
for(a in 1:length(slid1$dataslide[1,1,])){
  ...
  s1_p_E3<-project_to_curve(slid1$dataslide[c(fix[3]:(fix[4]-1)),,a],splines$EDJ3[[a]],stretch=0)
  s1_p_E4<-project_to_curve(slid1$dataslide[c(fix[4]:(fix[5]-1)),,a],splines$EDJ4[[a]],stretch=0)
  s1_p_C<-project_to_curve(slid1$dataslide[c(fix[5]:length(slid1$dataslide[,1,1])),,a],splines$CEJ[[a]],stretch=0)
  slid1_p[,,a]<-rbind(s1_p_E1$s,s1_p_E2$s,s1_p_E3$s,s1_p_E4$s,s1_p_C$s)
...
for(a in 1:length(slid2$dataslide[1,1,])){
  ...
  s2_p_E3<-project_to_curve(slid2$dataslide[c(fix[3]:(fix[4]-1)),,a],splines$EDJ3[[a]],stretch=0)
  s2_p_E4<-project_to_curve(slid2$dataslide[c(fix[4]:(fix[5]-1)),,a],splines$EDJ4[[a]],stretch=0)
  s2_p_C<-project_to_curve(slid2$dataslide[c(fix[5]:length(slid2$dataslide[,1,1])),,a],splines$CEJ[[a]],stretch=0)
  slid2_p[,,a]<-rbind(s2_p_E1$s,s2_p_E2$s,s2_p_E3$s,s2_p_E4$s,s2_p_C$s)
...
# UM1 colours
col<-c("#e41a1c","#377eb8","#4daf4a","black","black","#984ea3")
# UM2 colours
col<-c("#e41a1c","#377eb8","#4daf4a","black","black","#984ea3")
# UM3 colours
col<-c("#e41a1c","#377eb8","#4daf4a","black","black","#984ea3")
...
#LM2 colours
col<-c("#e41a1c","black","#377eb8","black","#4daf4a","#984ea3")
#LM3 colours
...
# Plot PCA PC1 V 3
plot(cbind(Proc$PCscores[,1],Proc$PCscores[,3]),type="n",asp=1,cex=1,xlab="PC 1",ylab="PC 3")
for(a in 1:length(levels(groups))){
  sub<-groups==levels(groups)[a]
  tr <- NULL
  ...
  if(!is.null(tr)){
    ...
    lines(Proc$PCscores[sub,1],Proc$PCscores[sub,3],col=col[a],lwd = 2)
  }}
points(cbind(Proc$PCscores[,1],Proc$PCscores[,3]),col=col[groups],pch=19)
text(cbind(Proc$PCscores[,1],Proc$PCscores[,3]),label=dimnames(Proc$PCscores)[[1]],col=col[groups],pos=c(1,2),cex=0.6,offset=0.5)
